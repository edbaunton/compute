...
struct meta_kernel_buffer_info
{
    meta_kernel_buffer_info(const buffer &buffer,
                            const std::string &id,
                            const memory_object::address_space &addr_space,
                            size_t i)
      : m_mem(buffer.get()),
        identifier(id),
...
public:
    ...
    template<class T>
    size_t add_arg(const memory_object::address_space address_space,
                   const std::string &name)
    {
        return add_arg_with_qualifiers<T>(address_space_string(address_space), name);
    }
    ...
    static const char* address_space_string(const memory_object::address_space value)
    {
        switch(value){
